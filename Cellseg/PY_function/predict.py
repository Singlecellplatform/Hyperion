...
def predict_feature_pad(weights, input_size, image, target_size, results_dir, image_shape):
   ...
   # print (
   #  model.predict(testGene, 1,verbose=1) 
   # )
...
def main(CONFIG):
    ...
    if (CONFIG.TEST_IMAGE != None):
        ...
        for i in range(len(watershed_list)):
            ...
            mask_final = (mask_final * 255).astype('uint8') if mask_final.dtype == np.float32 else mask_final.astype('uint8')
           
            print("Shape of mask:", mask_final.shape)
            print("Data type of mask:", mask_final.dtype)
            print("Min value in mask:", np.min(mask_final))
            print("Max value in mask:", np.max(mask_final))
